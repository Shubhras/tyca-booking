    ...
    <div id="slot_option">
        <div class="col-12 mb-3 form-group">
            ...
            <div class="fc-timegrid-slot pe-5 form-control form-control-solid h-300px overflow-auto">
                ...
                <div class="text-center d-flex flex-wrap appointment-slot-data" id="slotData">
                </div>
        ...
        <div class="row">
            <div class="col-lg-6 col-sm-12 mb-5">
                <div class="form-group">
                    <label class="form-label" for="first_name">{{ __('messages.patient.first_name') }}:<span class="required"></span></label>
                    <input type="text" class="form-control" id="first_name" placeholder="{{ __('messages.doctor.first_name') }}" name="first_name" value="">
                </div>
            </div>
            <div class="col-lg-6 col-sm-12 mb-5">
                <div class="form-group">
                    <label class="form-label" for="last_name">{{ __('messages.patient.last_name') }}:<span class="required"></span></label>
                    <input type="text" id="last_name" name="last_name" class="form-control" value="" placeholder="{{ __('messages.doctor.last_name') }}">
                </div>
            </div>
        </div>
        <div class="col-sm-12 col-lg-12 mb-5">
            <div class="form-group">
                <label class="form-label" for="email">{{ __('messages.patient.email') }}:<span class="required"></span></label>
                <input type="email" id="email" name="email" class="form-control" value="" placeholder="{{ __('messages.web.email') }}">
            </div>
        </div>
        ...
        <div class="col-xxl-6 mb-7 d-sm-flex align-items-center mb-3">
            <div class="col-xl-4 col-lg-4 col-md-2 col-4">
                <label class="form-check" style="    padding-left: 0em;">
                    <input class="form-check-input feature mx-2" type="radio" value="<?php echo $index;  ?>" name="payment_type" /><?php echo $value;  ?>
                </label>
            </div>
            <div class="col-lg-6">
                <?php if ($index == '4') { ?>
...
<div class="col-lg-6 col-sm-12 mb-5">
    ...
    <div class="input-group">
        ...
        <strong style="display: none;"><span id="price_total2">0</span></strong>

    </div>
</div>
<input type="hidden" id="pp_pp" name="backend_account">

<strong style="display: none;"><span id="getprice_value">0</span></strong>
<strong style="display: none;"><span id="getprice_days">0</span></strong>
...
<script type="text/javascript">
    $('body').on('change', '#plantype_id', function() {
        ...
        if (selectedText == 'hourly') {
            ...
            $("#dateShow").hide();
            $("#iii").append('')
            $('#price_total1').empty().text($('#getprice_value').text());
            var cc = $('#getprice_value').text();
            $("#pp_pp").empty().val(cc);

        } else {
            $("#dateHide").hide();
            $("#dateShow").show();
            $('#price_total1').empty().text($('#getprice_days').text());
            var cc = $('#getprice_days').text();
            $("#pp_pp").empty().val(cc);
            $("#iii").empty().append(
            ...
            $('body').on('click', '.applyBtn', function() {
                ...
                if (selectedText != 'hourly') {
                    var priceTotalElement = document.getElementById('price_total2');
                    var priceTotalValue = priceTotalElement.innerText;
                    var priceTotalNumber = parseInt(priceTotalValue);
                    ...
                    if (daysDiff == 0) {
                        ...
                        $('#price_total1').text(priceTotalNumber * daysDiff);
                        var cc = (priceTotalNumber * daysDiff);
                        $("#pp_pp").empty().val(cc);
