      ...
      if (result.success) {
        ...
        $.each(result.data['slots'], function (index, value) {
          ...
          if (result.data['bookedSlot'] == null) {
            ...
            if ($.inArray(value, result.data['bookedSlot']) !== -1) {
              $('.appointment-slot-data').append('<div><input type="checkbox" checked disabled class="time-alot  slots-item bg-success time-slot bookedSlot" data-id="' + value + '"><span class="badge badge-lg slots-item bg-success time-slot bookedSlot">' + value + '</span></div>');
            } else {
              $('.appointment-slot-data').append('<div><input type="checkbox" class="time-alot slots-item bg-success time-slot" data-id="' + value + '"> <span class="badge badge-lg slots-item bg-success time-slot">' + value + '</span></div>');
            }
          }
        });
      }
